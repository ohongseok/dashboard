...
+    activity_cols = ["D_listed", "E_req_done", "F_purchase_req", "G_purchase_done", "I_reg_done"]
+    df["has_activity"] = df[activity_cols].any(axis=1)
+    df["include_in_scope"] = (
+        df["등록요청일_dt"].ge(pd.Timestamp("2024-01-01")).fillna(False)
+        | df["has_activity"]
+        | df["is_불가"]
+    )
